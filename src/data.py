...
class ImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if(self.inmodal):
            ...
            item["pixel_values"] = self.processor.process_image(Image.open(os.path.join(self.images[idx]))), self.processor.process_image(_augment_image(os.path.join(self.images[idx])))
        else:  
            item["input_ids"] = self.captions["input_ids"][idx]
            item["attention_mask"] = self.captions["attention_mask"][idx]
            item["pixel_values"] = self.processor.process_image(Image.open(os.path.join(self.images[idx])))
...
class ImageLabelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = self.transform(Image.open(os.path.join(self.images[idx])))
